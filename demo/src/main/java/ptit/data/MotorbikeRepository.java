...
public interface MotorbikeRepository extends CrudRepository<Motorbike,Integer>{
    ...
    @Query(value="SELECT * FROM tblxe WHERE tblxe.bienso = ?1",nativeQuery = true)
    public Motorbike findOneByeMotorId(String bienso);


    @Query(value = "SELECT * FROM tblxe WHERE (NOT EXISTS (SELECT id FROM tblmonthly WHERE tblmonthly.xeid = tblxe.id))",nativeQuery = true)
    public List<Motorbike> findMotorbikeNotHavedTicketYet();


    @Transactional
    @Modifying
    @Query(value="DELETE FROM tblxe WHERE tblxe.sinhvienid = ?1",nativeQuery=true)
